...
REWRITE_CELL_TEST_DATA: Iterable[tuple[str, KInner, KInner]] = (
    (
        'rewrite-address',
        KRewrite(
            lhs=KToken(token='0', sort=KSort(name='Int')),
            rhs=KToken(token='728815563385977040452943777879061427756277306518', sort=KSort(name='Int')),
        ),
        KRewrite(
            lhs=KToken(token='0', sort=KSort(name='Int')),
            rhs=KToken(token='728815563385977040452943777879061427756277306518', sort=KSort(name='Int')),
                ...
                KApply(
                    ...
                    args=(
                        KApply(
                            ...
                            args=(
                                KRewrite(
                                    lhs=KToken(token='0', sort=KSort(name='Int')),
                                    rhs=KToken(
                                        token='645326474426547203313410069153905908525362434349', sort=KSort(name='Int')
                                    ),
                                ),
                            ),
                        ),
                        ...
                        KApply(
                            ...
                            args=(
                                KRewrite(
                                    lhs=KToken(token='b""', sort=KSort(name='Bytes')),
                                    rhs=KToken(token='b"\\x00"', sort=KSort(name='Bytes')),
                                ),
                            ),
                        ),
                        KApply(
                            ...
                            args=(
                                KRewrite(
                                    ...
                                    rhs=KVariable(name='CHEATCODE_STORAGE_FINAL', sort=KSort(name='Map')),
                                ),
                            ),
                        ),
                        KApply(
                        ...
                        KApply(
                            label=KLabel(name='<nonce>', params=()), args=(KToken(token='0', sort=KSort(name='Int')),)
                        ),
                    ),
                ),
                KApply(
                ...
                KApply(
                    ...
                    args=(
                        KApply(
                            ...
                            args=(
                                KToken(
                                    token='728815563385977040452943777879061427756277306518', sort=KSort(name='Int')
                                ),
                            ),
                        ),
                        KApply(
                        ...
                        KApply(
                            label=KLabel(name='<nonce>', params=()), args=(KToken(token='2', sort=KSort(name='Int')),)
                        ),
                    ),
                ),
                KApply(
...
class TestImpProof(KCFGExploreTest, KProveTest):
    ...
    @pytest.mark.parametrize(
        ...
        REWRITE_CELL_TEST_DATA,
        ids=[test_id for test_id, *_ in REWRITE_CELL_TEST_DATA],
    )
    def test_no_cell_rewrite_to_dots(
        self,
        kcfg_explore: KCFGExplore,
        test_id: str,
        term: KInner,
        ...
        result = kcfg_explore.no_cell_rewrite_to_dots(term)
