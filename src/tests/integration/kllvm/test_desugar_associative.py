...
from pyk.kllvm.convert import kore_pattern_to_llvm
...
def test_desugar_associative(kore_text: str, expected_text: str) -> None:
    # Given
    kore = kore_pattern_to_llvm(KoreParser(kore_text).pattern())
    expected = kore_pattern_to_llvm(KoreParser(expected_text).pattern())
