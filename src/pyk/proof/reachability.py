...
class APRProver(Prover):
    ...
    def _check_subsume(self, node: KCFG.Node) -> bool:
        ...
        if self.fast_check_subsumption and not self._may_subsume(node):
            ...
            return False
        csubst = self.kcfg_explore.cterm_symbolic.cterm_implies(
            node.cterm, self.proof.kcfg.node(self.proof.target).cterm
        )
...
class APRFailureInfo:
    ...
    def from_proof(proof: APRProof, kcfg_explore: KCFGExplore, counterexample_info: bool = False) -> APRFailureInfo:
        ...
        for node in proof.failing:
            node_cterm, _ = kcfg_explore.cterm_symbolic.cterm_simplify(node.cterm)
            target_cterm, _ = kcfg_explore.cterm_symbolic.cterm_simplify(target.cterm)
            ...
            if counterexample_info:
                model_subst = kcfg_explore.cterm_symbolic.cterm_get_model(node.cterm)
