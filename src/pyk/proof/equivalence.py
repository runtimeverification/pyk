...
from pyk.proof.exploration import ExplorationProof, ExplorationProver
...
class EquivalenceProof:
    ...
    proof_1: ExplorationProof
    proof_2: ExplorationProof
    ...
    def __init__(
        ...
        assert len(root_1) == 1 and len(root_2) == 1
        self.proof_1 = ExplorationProof(id_1, kcfg_1, root_1[0].id, logs_1, proof_dir=proof_dir_1)
        self.proof_2 = ExplorationProof(id_2, kcfg_2, root_2[0].id, logs_2, proof_dir=proof_dir_2)

    # Constructor from two proofs
    @staticmethod
    def from_proofs(proof_1: ExplorationProof, proof_2: ExplorationProof) -> EquivalenceProof:
    ...
    def read_proof(id_1: str, proof_dir_1: Path, id_2: str, proof_dir_2: Path) -> EquivalenceProof:
        ...
        proof_1 = ExplorationProof.read_proof(id_1, proof_dir_1)
        proof_2 = ExplorationProof.read_proof(id_2, proof_dir_2)
    ...
    def from_dict(cls: type[EquivalenceProof], dct: Mapping[str, Any]) -> EquivalenceProof:
        ...
        proof_1 = ExplorationProof.from_dict(dct['proof_1'])
        proof_2 = ExplorationProof.from_dict(dct['proof_2'])
        return EquivalenceProof.from_proofs(proof_1, proof_2)
    ...
    def summary(self) -> Iterable[str]:
        summary_1 = ['    ' + s for s in self.proof_1.summary.__str__().splitlines()]
        summary_2 = ['    ' + s for s in self.proof_2.summary.__str__().splitlines()]
        return (
            [
                'EquivalenceProof:',
    ...
    def check_equivalence(
        ...
        terminal_1 = [kcfg_1.node(id) for id in self.proof_1._terminal_node_ids]
        terminal_2 = [kcfg_2.node(id) for id in self.proof_2._terminal_node_ids]

        pc_terminal_1 = KCFG.multinode_path_constraint(terminal_1)
        pc_terminal_2 = KCFG.multinode_path_constraint(terminal_2)

        _LOGGER.info('Checking subsumption of path constraints of final states')
        ...
        _LOGGER.info('Checking subsumption of path constraints of pending states')
        ...
        # _LOGGER.info('Checking subsumption of path constraints of bounded states')
...
class EquivalenceProver:
    prover_1: ExplorationProver
    prover_2: ExplorationProver
    ...
    def __init__(
        ...
        self.prover_1 = ExplorationProver(proof.proof_1, kcfg_explore)
        self.prover_2 = ExplorationProver(proof.proof_2, kcfg_explore)
